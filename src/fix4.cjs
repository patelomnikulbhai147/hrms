...
const endTarget = '  // ─── HR Dashboard (HR Operations Dashboard) ───';

const startIndex = content.indexOf(startTarget);
const endIndex = content.indexOf(endTarget);

if (startIndex !== -1 && endIndex !== -1) {
    // preserve up to start, then add our new content, then add '  }\n\n' because our newContent replaces up to the end of the return statement, and we need to close the if statement before the next block!
    content = content.substring(0, startIndex) + newContent + '\n  }\n\n' + content.substring(endIndex);
    
    // Check if AnimatedCounter import is already there, if not, add it
    if (!content.includes('import AnimatedCounter from \'../components/common/AnimatedCounter\';')) {
