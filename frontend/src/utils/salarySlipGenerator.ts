...
// A payslip must read identically for the employee, HR and the auditor —
// utils/formatDate.ts pins the format and time zone (Asia/Kolkata).
...
export const buildPayslipDoc = (record: any, employee: any, company: any, attendanceSummary?: any): { doc: any; fileName: string } => {
  ...
  // Centralized branding (single source of truth). Drawn FIRST so the page
  // content (tables/text) overlays the faint watermark rather than hiding under it.
  const brand = resolveBranding(company);
  if (brand.hasWatermark) {
    try {
      (doc as any).saveGraphicsState?.();
      (doc as any).setGState?.(new (doc as any).GState({ opacity: 0.06 }));
      if (brand.watermarkImage && isRasterImage(brand.watermarkImage)) {
        const fmt = /png/i.test(brand.watermarkImage) ? 'PNG' : 'JPEG';
        doc.addImage(brand.watermarkImage, fmt, 45, 90, 120, 120);
      } else if (brand.watermarkText) {
        doc.setFontSize(60);
        doc.setFont('helvetica', 'bold');
        (doc as any).text(brand.watermarkText, 105, 160, { align: 'center', angle: 35 });
      }
      (doc as any).restoreGraphicsState?.();
    } catch (e) { /* watermark is decorative — never break the slip */ }
  }
  ...
  if (brandLogo && isRasterImage(brandLogo)) {
    try {
  ...
  doc.setFontSize(8);
  doc.setFont("helvetica", "normal");
  doc.text(`Generated Date: ${formatDateTime(new Date())}`, 14, finalY);
  ...
  // Authorized signature + company seal from central branding (right side).
  // If a signature image is configured, render it above a signatory line; else
  // fall back to the digital-verification note (backward compatible).
  let drewSignature = false;
  try {
    if (brand.hasSignature && isRasterImage(brand.signature)) {
      const fmt = /png/i.test(brand.signature) ? 'PNG' : 'JPEG';
      doc.addImage(brand.signature, fmt, 120, finalY, 40, 16);
      drewSignature = true;
    }
    if (brand.hasSeal && isRasterImage(brand.seal)) {
      const fmt = /png/i.test(brand.seal) ? 'PNG' : 'JPEG';
      doc.addImage(brand.seal, fmt, 95, finalY - 2, 22, 22);
    }
  } catch (e) { /* image assets are optional — never break the slip */ }

  doc.setFont("helvetica", drewSignature ? 'normal' : 'bold');
  if (drewSignature) {
    doc.setLineWidth(0.3);
    doc.line(120, finalY + 18, 160, finalY + 18);
    doc.setFontSize(8);
    doc.text(brand.signatureText || 'Authorized Signatory', 140, finalY + 22, { align: 'center' });
  } else {
...
export const generateEnterprisePayslipExcel = (record: any, employee: any, company: any, attendanceSummary?: any) => {
  ...
  wsData.push(['Generated Date:', formatDateTime(new Date())]);
