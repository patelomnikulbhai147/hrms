    ...
    <div>
      ...
      {canEdit && !pending && (
        <div
          ...
          className={`mb-2 cursor-pointer rounded-xl border border-dashed px-3 py-3 text-center transition ${drag ? 'border-[#C77E52] bg-[#FCF4EE]' : 'border-slate-200 hover:border-[#C77E52]/60'}`}>
          <p className="text-[11px] text-slate-500 flex items-center justify-center gap-1.5"><UploadCloud size={14} className="text-[#C77E52]" /> Drag &amp; drop a file here, or click to browse (PDF, DOCX, XLSX, JPG, PNG)</p>
        </div>
      )}

      {/* Pending upload form */}
      {pending && (
        <div className="mb-2 rounded-xl border border-[#F7E3D3] bg-[#FCF4EE] p-3 space-y-2">
          <div className="flex items-center gap-2 text-xs text-slate-600"><FileText size={14} className="text-[#C77E52]" /> <span className="font-semibold truncate">{pending.file.fileName}</span> <span className="text-slate-400">· {pending.file.size}</span>
