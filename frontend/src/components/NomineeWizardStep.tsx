...
import { UserPlus, Pencil, Trash2, ShieldAlert, Heart, Scale, FileText } from 'lucide-react';
...
export const NomineeWizardStep: React.FC<Props> = ({ value, onChange }) => {
  ...
  const save = (nominee: any, docs: NomineeDoc[]) => {
    if (editIdx === -1) {
      onChange([...value, { ...nominee, documents: docs }]);
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg bg-blue-50 border border-blue-200 px-3 py-2 text-[11px] text-blue-800">
        Add one or more nominees (optional). They are saved <strong>only after</strong> the employee is created — together, in a single transaction. Total allocation should equal <strong>100%</strong>.
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3">
        ...
        {value.length > 0 && total > 100 && <p className="text-[10px] text-rose-600 mt-1.5 font-semibold">⚠ Total exceeds 100%. Reduce some allocations.</p>}
        {value.length > 0 && total < 100 && <p className="text-[10px] text-amber-600 mt-1.5 font-semibold">⚠ Total is {total}% — should equal 100%.</p>}
      </div>
      ...
      <NomineeForm
        ...
        existingDocsCount={editIdx != null && editIdx >= 0 ? value[editIdx]?.documents?.length : 0}
