    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search templates…" className="w-full pl-8 pr-3 py-2 text-xs rounded-lg border border-slate-200 focus:border-[#C77E52] outline-none" />
        </div>
        <div className="flex flex-wrap gap-1">
          {cats.map((c) => (
            <button key={c} onClick={() => setCat(c)} className={`px-2.5 py-1.5 text-[11px] font-bold rounded-lg border ${cat === c ? 'bg-[#C77E52] text-white border-[#C77E52]' : 'bg-white text-slate-500 border-slate-200 hover:text-slate-700'}`}>{c}</button>
          ))}
        </div>
      </div>
