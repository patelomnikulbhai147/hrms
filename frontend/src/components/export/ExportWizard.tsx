...
const selCls = 'w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 focus:outline-none focus:border-[#4F7CFF] transition-colors';
const inpCls = selCls;
...
const Chk: React.FC<{ checked: boolean; onChange: () => void; label: string; disabled?: boolean }> = ({ checked, onChange, label, disabled }) => (
  <button type="button" onClick={onChange} disabled={disabled}
    ...
    {checked ? <CheckSquare size={15} className="text-[#4F7CFF]" /> : <Square size={15} className="text-slate-300" />}{label}
  </button>
);
const Radio: React.FC<{ active: boolean; onClick: () => void; label: string }> = ({ active, onClick, label }) => (
  <button type="button" onClick={onClick}
    className={`px-3 py-1.5 rounded-lg text-[11px] font-bold border transition-colors ${active ? 'bg-[#EDF4FF] border-[#4F7CFF] text-[#4F7CFF]' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}>{label}</button>
);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                {FORMATS.map(f => (
                  <button key={f.id} type="button" onClick={() => setFormat(f.id)}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-xs font-bold transition-colors ${format === f.id ? 'bg-[#EDF4FF] border-[#4F7CFF] text-[#4F7CFF]' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                    {f.icon}{f.label}
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-3.5 space-y-1.5">
              ...
              <SummaryRow label="Format" value={<span className="uppercase font-bold text-[#4F7CFF]">{format}</span>} />
...
const PreviewPane: React.FC<{ meta: ReportMeta; columns: ExportColumn[]; rows: any[]; format: string; attachments: boolean; attachCount: number }> = ({ meta, columns, rows, format, attachments, attachCount }) => {
  ...
  return (
    <div>
      <div className="flex items-start justify-between gap-4 border-b-2 border-[#4F7CFF] pb-3 mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm font-bold text-[#4F7CFF]">{meta.title}</div>
      ...
      <div className="overflow-auto rounded-lg border border-slate-200 max-h-[52vh]">
        <table className="w-full text-[11px] border-collapse">
          <thead className="sticky top-0"><tr className="bg-[#4F7CFF] text-white">{columns.map(c => <th key={c.key} className="text-left font-bold px-2.5 py-2 whitespace-nowrap">{c.header}</th>)}</tr></thead>
          <tbody>
