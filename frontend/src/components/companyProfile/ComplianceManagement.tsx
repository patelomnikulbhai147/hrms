      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search all company documents…" className="pl-9 pr-3 py-2 text-xs rounded-xl border border-slate-200 bg-white text-slate-700 w-60 focus:outline-none focus:border-[#C77E52]" />
          </div>
          <select value={catFilter} onChange={e => setCatFilter(e.target.value)} className="py-2 px-3 text-xs rounded-xl border border-slate-200 bg-white text-slate-700 focus:outline-none focus:border-[#C77E52]">
            <option value="All">All Categories</option>
            {renderCats.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
          </select>
          <div className="flex items-center gap-1 flex-wrap">
            {VISIBLE_STATUS_FILTERS.map(f => (
              <button key={f} onClick={() => setFilter(f)} className={`px-2.5 py-1.5 text-[11px] font-bold rounded-lg transition-colors ${filter === f ? 'bg-[#FCF4EE] text-[#C77E52]' : 'text-slate-500 hover:bg-slate-100'}`}>{f}</button>
            ))}
          </div>
        </div>
                ...
                {items.map(i => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                      <button onClick={() => previewFile(i.record)} className="text-slate-400 hover:text-[#C77E52]" title="View document"><Eye size={15} /></button>
                      {i.record?.fileData && <button onClick={() => downloadFile(i.record.fileData, i.record.fileName)} className="text-slate-400 hover:text-[#C77E52]" title="Download certificate"><Download size={15} /></button>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <div className="mt-1.5 flex items-center gap-2 flex-wrap">
                ...
                {draft.fileData && <button onClick={() => downloadFile(draft.fileData, draft.fileName)} className="text-slate-400 hover:text-[#C77E52]"><Download size={14} /></button>}
    ...
    <div className="space-y-2">
      ...
      {versions.map((v, idx) => (
        <div key={idx} className="flex items-center justify-between rounded-lg bg-slate-50 border border-slate-200 px-3 py-2">
          ...
          <button onClick={() => onDownload(v.fileData, v.fileName)} className="text-slate-400 hover:text-[#C77E52]"><Download size={14} /></button>
        </div>
      ))}
    </div>
  );
