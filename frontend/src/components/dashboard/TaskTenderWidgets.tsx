...
interface Props {
  ...
  // Tender Management is leadership-only (Super Admin / Company Head). When false,
  // the Tender Information panel is hidden entirely and no tender API is called.
 ...
 */
export const TaskTenderWidgets: React.FC<Props> = ({ activeCompanyId, onNavigate, canViewTenders = false }) => {
  ...
  useEffect(() => {
    ...
    // Only leadership may read tenders (the module is leadership-gated); skip the
    // call entirely for everyone else so no 403 is triggered.
    if (canViewTenders) api.tenders.getAll().then(d => { if (alive) setTenders(d || []); }).catch(() => {});
    return () => { alive = false; };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      {/* Tender Information widget — leadership only (Super Admin / Company Head) */}
      {canViewTenders && (
