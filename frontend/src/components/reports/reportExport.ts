  ...
  w.document.write(`<!doctype html><html><head><title>${esc(title)}</title>
    ...
    <style>
      ...
      * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
      ...
      /* Closing block (Remarks → Signature → Generated By → Footer) stays whole
         and only moves to a new page when it truly does not fit. */
...
/**
 * SMART PAGE BREAK — the single pagination brain shared by every PDF report.
 *
 * Pure function (no DOM, no jsPDF) so it can be reasoned about and tested. Given
 * the rasterised content height and the list of ATOMIC blocks (table rows plus
 * any [data-keep-together] group such as Remarks → Signature → Footer), it
 * returns the page slices as [startPx, endPx, isFirstPage].
 *
 * The rule: fill each page to the very bottom. Retreat from the page boundary
 * ONLY when the cut line falls inside an atomic block, and then only as far as
 * that block's top. The previous engine always retreated to the last table-row
 * edge, which is what left a large blank area under the table and pushed the
 * remarks/signature/footer onto a needless extra page.
 */
export function planPages(opts: {
  contentPx: number;              // total rasterised content height, canvas px
  px2pt: number;                  // canvas px → PDF pt
  pageH: number;                  // page height, pt
  mT: number; mB: number;         // top/bottom margins, pt
  footerH: number;                // reserved running-footer band, pt
  contHeaderH: number;            // continuation header on pages after the first, pt
  atomic: Array<[number, number]>; // unbreakable [top, bottom] spans, canvas px
}): Array<[number, number, boolean]> {
  const { contentPx, px2pt, pageH, mT, mB, footerH, contHeaderH, atomic } = opts;
  const slices: Array<[number, number, boolean]> = [];
  const EPS = 2;                                        // px — ignore hairline diffs
  let rendered = 0;
  let pageNo = 0;
  while (rendered < contentPx - EPS) {
    const isFirst = pageNo === 0;
    const usableHpt = pageH - mT - mB - footerH - (isFirst ? 0 : contHeaderH);
    const maxSlicePx = usableHpt / px2pt;
    // Absorb a tiny remainder rather than spawning a near-empty extra page. The
    // overshoot (≤2% of the page) lands in the bottom margin / page padding.
    const orphanTol = maxSlicePx * 0.02;
    const target = rendered + maxSlicePx;
    let end: number;
    if (target >= contentPx - orphanTol) {
      end = contentPx;
    } else {
      let cut = target;
      for (let pass = 0; pass < 50; pass++) {
        let moved = false;
        for (const [t, b] of atomic) {
          if (t > rendered + EPS && t < cut - EPS && b > cut) { cut = t; moved = true; }
        }
        if (!moved) break;                              // fixpoint (blocks may nest)
      }
      // A block taller than a whole page cannot be kept intact — hard cut.
      end = cut > rendered + EPS ? cut : target;
    }
    slices.push([rendered, end, isFirst]);
    rendered = end;
    pageNo++;
    if (pageNo > 1000) break;                           // pathological-content guard
  }
  if (!slices.length) slices.push([0, contentPx, true]);
  return slices;
}

// Rasterise the report into a multi-page A4 PDF that matches the preview. Pages
// break only where they must (see planPages), and every page carries a running
// header (company — title) plus a footer (generated date/by + page x/y).
export async function nodeToPdf(node: HTMLElement, fileName: string, orientation: Orientation = 'portrait', ctx: ExportContext = {}) {
  // html2canvas-pro (NOT the original html2canvas) — Tailwind v4 emits modern
  // `oklch()` colours, which the original parser cannot read: it throws
  // "Attempting to parse an unsupported color function \"oklch\"" and the PDF
  // comes out blank/broken. The -pro fork understands them. Every other export
  // path in this app already uses -pro; this one was the last hold-out.
  ...
  // A self-paged doc (.report-doc) IS an A4 sheet and already carries its own
  // 15mm/10mm padding. Adding the page margins again would make a 297mm document
  // taller than the 297mm page, so a one-page report spilled a sliver onto page 2
  // — and everything after the last table row (remarks, signature, seal, footer)
  // went with it, leaving page 1 half empty. Print already made this distinction;
  // the PDF path did not. Now they agree.
  const selfPaged = !!node.classList?.contains('report-doc');
  const mL = selfPaged ? 0 : PAGE.marginLeft * PT_PER_MM;
  const mR = selfPaged ? 0 : PAGE.marginRight * PT_PER_MM;
  const mT = selfPaged ? 0 : PAGE.marginTop * PT_PER_MM;
  const mB = selfPaged ? 0 : PAGE.marginBottom * PT_PER_MM;
  // Reserved bands. A self-paged doc reserves nothing: its own bottom padding is
  // white space we can draw the footer into, so the footer costs no page height.
  const footerH = selfPaged ? 0 : 14;
  const contHeaderH = selfPaged ? 0 : ((ctx.companyName || ctx.title) ? 16 : 0);
  const usableW = pageW - mL - mR;
  const px2pt = usableW / canvas.width;                 // canvas px → pt (full width)

  // ── Atomic blocks: content that must never be split across a page break ──
  // Every table row, plus anything the report marks [data-keep-together] (the
  // Remarks → Signature → Generated By → Footer group). Measured in canvas px
  // from the top of the node.
  const nodeTop = node.getBoundingClientRect().top;
  const atomic: Array<[number, number]> = [];
  node.querySelectorAll('tr, [data-keep-together]').forEach((el) => {
    const r = el.getBoundingClientRect();
    const top = (r.top - nodeTop) * scale;
    const bottom = (r.bottom - nodeTop) * scale;
    if (bottom > top && bottom > 0 && top < canvas.height) atomic.push([Math.max(0, top), Math.min(canvas.height, bottom)]);
  });

  const slices = planPages({ contentPx: canvas.height, px2pt, pageH, mT, mB, footerH, contHeaderH, atomic });
  ...
  slices.forEach(([start, end, isFirst], i) => {
    ...
    // Baseline of the running footer. Unchanged for margin-paged reports; for a
    // self-paged sheet it sits 6mm from the edge — inside the document's own
    // bottom padding, so it consumes no content height.
    const footY = pageH - (selfPaged ? 6 * PT_PER_MM : mB - 10);
